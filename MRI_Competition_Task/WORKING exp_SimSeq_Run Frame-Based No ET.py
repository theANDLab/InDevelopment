...
trials_column_order = ['welcome.start','instructions.start', 'instructions.end', 'blank_block.start', 'blank_block.end', 'feat_cond', 'run', 'attention_cond','block',
...
def run_blank_block(feat_cond, attn_cond, run_num, blank_num, rsvp, rsvp_state, last_target_onset): 
    """ 
    ...
    # Add start of block to data files
...
def run_trial(feat_cond, attention_cond, run, rsvp, rsvp_state, trial_dict, last_target_onset):    
    ...
    rsvp_sequence = trial_dict['rsvp_seq'] # only to add pokemon displayed during the trial to the data file
    RATE = RSVP_RATE
    frames_per_pokemon = round(RATE*frame_rate) 
    ...
    # Get current RSVP index
    rsvp_idx = rsvp_state['idx']
    rsvp_frame_counter = rsvp_state['frame']
    ...
    for frameN in range(0, total_trial_frames):
        ...
        if rsvp_frame_counter % frames_per_pokemon == 0:
        ...
        def on_flip():
            ...
            nonlocal last_target_onset, target_onset_recorded, flip_time, rsvp_idx, RATE
            
            flip_time = globalClock.getTime(format='float') # get time when window flipped
            
#            # Record time of first window flip of the trial
#            if frameN == 0:
#                thisExp.addData('trial.start', flip_time)
            ...
            if is_seq_trial:
                ...
                elif record_pstim_offset and not pstim_offset_recorded_dict[current_pstim.name]:
            ...
            if record_pokemon_onset:
                ...
                # Then start a NEW row for the CURRENT pokemon
        ...
        rsvp_frame_counter +=1
        rsvp_state['frame'] = rsvp_frame_counter
        rsvp_state['idx'] = rsvp_idx
